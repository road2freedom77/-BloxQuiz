...
const ROBUX_AMAZON_URL = "https://www.amazon.com/s?k=roblox+figures+phatmojo&tag=bloxquiz-20";

const VIBES = [
  { id: "chill",       label: "Chill & Relaxing",    emoji: "🌿", description: "Low stakes, cozy, hang out",       genres: ["simulator", "roleplay", "fashion"],                                                                              why: "These games let you play at your own pace — no pressure, no timers. Perfect for winding down or hanging out with friends." },
  { id: "action",      label: "Action & Combat",      emoji: "⚔️", description: "Fight, grind, level up",           genres: ["rpg", "shooter", "sports"],                                                                                      why: "Fast-paced games with real stakes — PvP, grinding systems, and builds that reward time invested. High player counts mean active servers right now." },
  { id: "horror",      label: "Horror & Suspense",    emoji: "😱", description: "Scary, tense, survival",           genres: ["horror", "mystery", "survival"],                                                                                  why: "Tension, jump scares, and unpredictable events. Best played with friends — or alone if you're brave enough." },
  { id: "competitive", label: "Competitive",           emoji: "🏆", description: "Win, rank up, outplay",            genres: ["tower-defense", "shooter", "sports", "obby"],                                                                    why: "Games where skill actually matters. High player counts mean you'll find a match fast and always have someone to beat." },
  { id: "social",      label: "Creative & Social",    emoji: "🎨", description: "Roleplay, express yourself",       genres: ["roleplay", "rhythm", "fashion"],                                                                                  why: "Less about winning, more about vibing. Customize, create, and connect with other players in open-ended worlds." },
  { id: "quick",       label: "Quick Session",         emoji: "⚡", description: "In and out, fast rounds",          genres: ["obby", "mystery", "shooter"],                                                                                    why: "Short rounds, instant action. Jump in, play a few rounds, and leave — no long-term commitment required." },
  { id: "deep",        label: "Deep Progression",     emoji: "🕹️", description: "Long grind, big rewards",          genres: ["rpg", "simulator", "tower-defense"],                                                                              why: "Games with real depth — skill trees, upgrades, and progression systems that keep you coming back for more. Best for longer sessions." },
  { id: "trending",    label: "What's Trending",      emoji: "🔥", description: "Most popular right now",           genres: ["rpg", "roleplay", "simulator", "shooter", "horror", "survival", "mystery", "obby", "tower-defense", "sports", "rhythm", "fashion"], why: "The games with the most momentum right now — Rising trend, high player counts, active communities. Jump in while they're hot.", trendingOnly: true },
  { id: "robux",       label: "Need More Robux?",     emoji: "💎", description: "Toys, figures, deals",             genres: [],                                                                                                                why: "" },
];
...
function FollowCTA({ gameSlug }: { gameSlug: string }) {
  ...
  async function handleFollow() {
    ...
    setLoading(true);
    const res  = await fetch("/api/games/follow", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: gameSlug }) });
  ...
  if (!user) {
    return (
      <SignInButton mode="modal" fallbackRedirectUrl="/what-roblox-game-should-i-play">
        <button style={{ background: "none", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 100, padding: "6px 14px", fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.5)", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>🔔 Follow</button>
      </SignInButton>
    );
  }

  if (following) return <span style={{ fontSize: 11, fontWeight: 700, color: "#00f5a0", padding: "6px 0" }}>✅ Following</span>;

  return (
    <button onClick={handleFollow} disabled={loading} style={{ background: "none", border: "1px solid rgba(0,245,160,0.3)", borderRadius: 100, padding: "6px 14px", fontSize: 11, fontWeight: 700, color: "#00f5a0", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", opacity: loading ? 0.6 : 1 }}>
      🔔 Follow
    </button>
  );
}
...
function RobuxPanel() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 28 }}>
      <a href={ROBUX_AMAZON_URL} target="_blank" rel="noopener noreferrer sponsored" style={{ textDecoration: "none" }}>
        <div style={{ background: "linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)", border: "1px solid rgba(255,215,0,0.25)", borderLeft: "3px solid #ffd700", borderRadius: 14, padding: "24px 28px", cursor: "pointer" }}>
          <div style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", color: "#ffd700", marginBottom: 10 }}>🧸 Roblox Toys &amp; Figures on Amazon</div>
          <div style={{ fontSize: 22, fontWeight: 900, color: "#fff", marginBottom: 6 }}>Phatmojo, Blox Fruits, Adopt Me &amp; More</div>
          <div style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", lineHeight: 1.6, marginBottom: 16 }}>
            Official Roblox figures and toy sets on Amazon. Free shipping with Prime. Great for collectors and fans of your favorite Roblox games.
          </div>
      ...
      <div style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "18px 20px" }}>
        <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 10 }}>Popular Roblox toy lines</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[
            "Phatmojo mystery figures — random characters from top games",
            "Blox Fruits action figures and accessories",
            "Adopt Me plush toys and collectibles",
            "Official Roblox branded figure sets",
          ].map((item, i) => (
      ...
      <div style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", fontWeight: 600 }}>Know what game you want to play?</div>
...
export default function GameFinderClient({ games }: { games: GameEntry[] }) {
  ...
  function handleVibe(vibeId: string) { setSelectedVibe(vibeId); setHasSearched(true); setTrendingOnly(false); }
  function reset() { setSelectedVibe(null); setHasSearched(false); setTrendingOnly(false); }
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a14", color: "#fff", fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 20px 80px" }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
            {VIBES.map((vibe) => {
              ...
              return (
                <button key={vibe.id} onClick={() => handleVibe(vibe.id)} style={{
                  background: isRobux
                    ? isSelected ? "linear-gradient(135deg, rgba(255,215,0,0.2), rgba(255,165,0,0.15))" : "linear-gradient(135deg, rgba(255,215,0,0.06), rgba(255,165,0,0.04))"
                    : isSelected ? "linear-gradient(135deg, rgba(0,245,160,0.15), rgba(184,76,255,0.15))" : "#111827",
                  border: isRobux
                  ...
                  borderRadius: 14, padding: "18px 16px", textAlign: "left", cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s",
                }}>
                  <div style={{ fontSize: 28, marginBottom: 8 }}>{vibe.emoji}</div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: isRobux ? (isSelected ? "#ffd700" : "rgba(255,215,0,0.8)") : isSelected ? "#00f5a0" : "#fff", marginBottom: 4 }}>{vibe.label}</div>
        ...
        {selectedVibe && !isRobuxVibe && selectedVibe !== "trending" && (
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 32 }}>
            <button onClick={() => setTrendingOnly(!trendingOnly)} style={{
              ...
              color: trendingOnly ? "#00f5a0" : "rgba(255,255,255,0.6)", cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s",
            }}>
        ...
        {hasSearched && (
          <div>
                ...
                {results.length === 0 ? (
                  ...
                  <>
                    <div style={{ background: "linear-gradient(135deg, #0f1629 0%, #111827 100%)", border: "1px solid rgba(0,180,216,0.2)", borderLeft: "3px solid #00b4d8", borderRadius: 12, padding: "18px 24px", marginBottom: 20 }}>
                      ...
                      <p style={{ margin: 0, fontSize: 14, color: "rgba(255,255,255,0.7)", lineHeight: 1.6 }}>{selectedVibeData?.why}</p>
                    </div>
                      ...
                      {results.map((game, i) => (
                        <div key={game.slug} style={{
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", marginBottom: 14 }}>
                            <div style={{ fontSize: 20, minWidth: 28, textAlign: "center" }}>{i === 0 ? "🏆" : `#${i + 1}`}</div>
                            {game.thumbnail_url && (
                            ...
                            <div style={{ flex: 1, minWidth: 140 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 3 }}>
                                <span style={{ fontSize: 16, fontWeight: 800, color: "#fff" }}>
                                  {game.emoji && <span style={{ marginRight: 4 }}>{game.emoji}</span>}{game.name}
                                </span>
                                {game.trend_label && (
                            ...
                            <FollowCTA gameSlug={game.slug} />
                          </div>
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            <a href={`/games/${game.slug}`}           style={{ background: "linear-gradient(135deg, #00b4d8, #0077b6)", color: "#fff", fontWeight: 800, fontSize: 12, padding: "8px 16px", borderRadius: 100, textDecoration: "none", whiteSpace: "nowrap" }}>🎮 Take a Quiz</a>
                            <a href={`/stats/${game.slug}`}           style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.7)", fontWeight: 700, fontSize: 12, padding: "8px 16px", borderRadius: 100, textDecoration: "none", whiteSpace: "nowrap" }}>📊 Live Stats</a>
                            <a href={`/codes/${game.slug}`}           style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.7)", fontWeight: 700, fontSize: 12, padding: "8px 16px", borderRadius: 100, textDecoration: "none", whiteSpace: "nowrap" }}>🎁 Codes</a>
                            <a href={`/stats/${game.slug}/history`}   style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.7)", fontWeight: 700, fontSize: 12, padding: "8px 16px", borderRadius: 100, textDecoration: "none", whiteSpace: "nowrap" }}>📈 History</a>
                          </div>
                        </div>
                      ))}
                    </div>

                    <a href={ROBUX_AMAZON_URL} target="_blank" rel="noopener noreferrer sponsored"
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", fontWeight: 600 }}>Phatmojo, Blox Fruits, Adopt Me — free Prime shipping</div>
                      </div>
            ...
            <div style={{ textAlign: "center", marginBottom: 8 }}>
              <button onClick={reset} style={{ background: "none", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 100, padding: "10px 24px", fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,0.5)", cursor: "pointer", fontFamily: "inherit" }}>
        ...
        <div style={{ marginTop: 32, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 32 }}>
          <div style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", color: "rgba(255,255,255,0.3)", marginBottom: 16, textAlign: "center" }}>Explore More</div>
