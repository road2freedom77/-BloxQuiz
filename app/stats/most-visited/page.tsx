    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: "Most Visited", item: "https://www.bloxquiz.gg/stats/most-visited" },
        ],
      },
      {
        "@type": "Article",
        headline: `Most Visited Roblox Games All Time — ${month}`,
        dateModified: new Date().toISOString(),
        author: { "@type": "Organization", name: "BloxQuiz" },
        publisher: { "@type": "Organization", name: "BloxQuiz" },
        description: "All-time ranking of Roblox games by total visits, updated hourly.",
        mainEntityOfPage: "https://www.bloxquiz.gg/stats/most-visited",
      },
      {
        "@type": "ItemList",
        name: `Most Visited Roblox Games — ${month}`,
        ...
        <div style={{ background: "linear-gradient(180deg, #0f0f23 0%, #0a0a14 100%)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "48px 0 40px" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 20px" }}>
            <nav style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", marginBottom: 24, display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <span>/</span>
              <span style={{ color: "rgba(255,255,255,0.7)" }}>Most Visited</span>
            </nav>
            ...
            <p style={{ margin: "0 0 28px", fontSize: 16, color: "rgba(255,255,255,0.55)", maxWidth: 600 }}>
              {topGame
                ? `${topGame.name} is the most visited Roblox game of all time with ${formatVisits(topGame.total_visits)} total visits. Rankings updated hourly via the Roblox API.`
                : "All-time rankings of the most visited Roblox games by total play count. Updated hourly."}
            </p>

            {topGame && (
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                <div style={{ background: "linear-gradient(135deg, #00b4d8 0%, #0077b6 100%)", borderRadius: 12, padding: "16px 24px", minWidth: 180 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(255,255,255,0.8)", marginBottom: 4 }}>#1 Most Visited</div>
                  <div style={{ fontSize: 20, fontWeight: 900, color: "#fff" }}>{topGame.emoji} {topGame.name}</div>
                  <div style={{ fontSize: 14, color: "rgba(255,255,255,0.8)", marginTop: 2 }}>{formatVisits(topGame.total_visits)} visits</div>
                </div>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 20px 80px" }}>
          ...
          <div style={{ background: "linear-gradient(135deg, #0f1629 0%, #111827 100%)", border: "1px solid rgba(0,180,216,0.2)", borderLeft: "3px solid #00b4d8", borderRadius: 12, padding: "20px 24px", marginBottom: 32 }}>
            <p style={{ margin: 0, fontSize: 15, lineHeight: 1.7, color: "rgba(255,255,255,0.85)" }}>
              {topGame
                ? `The most visited Roblox game of all time is ${topGame.name} with ${formatVisits(topGame.total_visits)} total visits as of ${month}. Total visits measure how many times the game has been played since launch — unlike concurrent players, this number only ever goes up. BloxQuiz tracks visit counts hourly so the data on this page is always up to date.`
                : "BloxQuiz tracks all-time visit counts for the most popular Roblox games, updated every hour via the Roblox API."}
            </p>
          </div>

          <div style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "52px 1fr 160px 140px 120px", padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)" }}>
              <span>#</span>
              <span>Game</span>
            ...
            {games.map((game, i) => (
              <Link key={game.slug} href={`/stats/${game.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block" }}>
                <div className="stats-row" style={{ display: "grid", gridTemplateColumns: "52px 1fr 160px 140px 120px", padding: "14px 20px", borderBottom: i < games.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", alignItems: "center" }}>
                  ...
                  <span style={{ textAlign: "right", fontSize: 15, fontWeight: 800, color: "#00b4d8", fontVariantNumeric: "tabular-nums" }}>{formatVisits(game.total_visits)}</span>
                  <span style={{ textAlign: "right", fontSize: 14, fontWeight: 600, color: "rgba(255,255,255,0.6)", fontVariantNumeric: "tabular-nums" }}>{formatNumber(game.current_players)}</span>
          ...
          <p style={{ marginTop: 20, fontSize: 12, color: "rgba(255,255,255,0.25)", textAlign: "center" }}>
            Visit counts fetched hourly from the Roblox API · Click any game for full stats &amp; charts
          </p>

          <div style={{ marginTop: 40, display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
            <Link href="/stats/most-played" style={{ background: "#111827", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "12px 20px", textDecoration: "none", color: "rgba(255,255,255,0.7)", fontWeight: 600, fontSize: 14 }}>🎮 Most Played Games</Link>
