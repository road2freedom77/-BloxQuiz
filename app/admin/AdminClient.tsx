...
export default function AdminClient({ quizzes, stats, flags, topQuizzes }: {
  quizzes: any[],
  stats: any,
  flags: any[],
  topQuizzes: any[],
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px", position: "relative", zIndex: 1 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
        {[
          ...
          { label: "Open Flags", value: stats.totalFlags, color: "var(--neon-yellow)" },
      ...
      {tab === "overview" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 24 }}>
            ...
            {flags.length === 0 ? (
              <p style={{ color: "var(--text-muted)", fontWeight: 600 }}>No flags yet.</p>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {flags.slice(0, 8).map((f, i) => (
                  <div key={i} style={{ padding: "10px 14px", background: "var(--surface)", borderRadius: "var(--radius-sm)" }}>
      ...
      {tab === "quizzes" && (
        <div>
            ...
            {filtered.map((quiz, i) => {
              ...
              return (
                <div key={quiz.slug} style={{ display: "grid", gridTemplateColumns: "1fr 140px 90px 60px 100px", alignItems: "center", padding: "12px 20px", borderBottom: i < filtered.length - 1 ? "1px solid var(--border)" : "none" }}>
                  ...
                  <div>
                    <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 100, background: diff.bg, color: diff.color }}>{quiz.difficulty}</span>
                  </div>
          ...
          <div style={{ marginTop: 12, fontSize: 13, fontWeight: 700, color: "var(--text-dim)" }}>
      ...
      {tab === "flags" && (
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden" }}>
          {flags.length === 0 ? (
            ...
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 60px 1fr 100px 80px", padding: "10px 20px", borderBottom: "1px solid var(--border)", fontSize: 11, fontWeight: 900, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: 1 }}>
                <div>Quiz</div>
                <div>Q#</div>
                <div>Reason</div>
                <div>Date</div>
              ...
              {flags.map((f, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px 1fr 100px 80px", alignItems: "center", padding: "14px 20px", borderBottom: i < flags.length - 1 ? "1px solid var(--border)" : "none" }}>
                  <div>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <button style={{ fontSize: 11, fontWeight: 800, padding: "4px 10px", borderRadius: 100, background: "rgba(0,245,160,0.1)", color: "var(--neon-green)", border: "none", cursor: "pointer" }}>
                      Dismiss
                    </button>
                  </div>
                </div>
