...
export async function generateMetadata({ params }: { params: Promise<{ game: string }> }) {
  ...
  return {
    ...
    description: `All active ${data.game} codes for 2026. Redeem these codes for free rewards, XP boosts, and more. Updated ${data.updatedAt}.`,
        ...
        {activeCodes.map(c => (
          <div key={c.code} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            ...
            <button
              ...
              style={{ background: "var(--gradient-main)", color: "var(--bg)", border: "none", borderRadius: 100, padding: "8px 18px", fontSize: 12, fontWeight: 800, cursor: "pointer", fontFamily: "var(--font-body)", WebkitTextFillColor: "var(--bg)" }}
            >
              Copy
            </button>
          </div>
